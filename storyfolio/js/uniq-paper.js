...
var B, Colors, G, PMill, R, RGBtoYST, TexGenXYRepeatUnit, W, Wb, Wg, Wr, YSTtoRGB, clampSat, det, gamma, inUnit, linear_to_monitor, matvecmul, maxYSTsat, offset, smallInteger, smallIntegerG0;
...
smallIntegerG0 = function() {
  return Math.abs(int(0.5 * nextGaussian())) + 1;
};

smallInteger = function() {
  return Math.abs(int(1.5 * nextGaussian()));
};

TexGenXYRepeatUnit = (function() {
  function TexGenXYRepeatUnit(vecs) {
    var angle, angle_stddev, as, at, lendiff_mean, lendiff_stddev, m, m0, rs, rt, scale, scale_log_stddev;
    scale = .3;
  ...
  TexGenXYRepeatUnit.prototype._getSTVectors = function() {
    "Get the 2 4-component vectors that (x,y,0,1) should\nbe multiplied by to get (s,t).";
    var f, mat, r;
    mat = this.vecs;
    f = 1.0 / (mat[0][0] * mat[1][1] - mat[0][1] * mat[1][0]);
    r = [[f * mat[1][1], -f * mat[0][1], 0, "?"], [-f * mat[1][0], f * mat[0][0], 0, "?"]];
    r[0][3] = Math.random();
    r[1][3] = Math.random();
    return r;
  };

  TexGenXYRepeatUnit.prototype.texCoords2D = function() {
    ...
    var stv;
    stv = this._getSTVectors();
    return [stv[0][0], stv[0][1], stv[0][2], stv[0][3], stv[1][0], stv[1][1], stv[1][2], stv[1][3]];
  };

  TexGenXYRepeatUnit.prototype.getRelated = function() {
    var a, abcd, b, c, chooseInts, d, f, vecs;
    chooseInts = function() {
      var a, abcd, b, c, d, i, l1, l2;
      while (1) {
        abcd = (function() {
          var k, results;
          results = [];
          for (i = k = 0; k < 4; i = ++k) {
            results.push(Math.round(2 * Math.pow(nextGaussian(), 3)));
          }
          return results;
        })();
        a = abcd[0];
    ...
    d = abcd[3];
    f = 1.0 / (a * d - b * c);
    vecs = [[f * d * this.vecs[0][0] - f * b * this.vecs[0][1], -f * c * this.vecs[0][0] + f * a * this.vecs[0][1]], [f * d * this.vecs[1][0] - f * b * this.vecs[1][1], -f * c * this.vecs[1][0] + f * a * this.vecs[1][1]]];
    if (0) {
      print("S: ", [this.vecs[0][0], this.vecs[1][0]]);
      print("T: ", [this.vecs[0][1], this.vecs[1][1]]);
...
NamedTextures = (function() {
  ...
  NamedTextures.prototype.get_names = function() {
    return ["rgbw1", "rgbw2", "rgbw3", "turb", "pyramid", "cone", "saw", "triangle", "rnd0", "rnd1", "rnd2"];
  };
  ...
  function NamedTextures(gl) {
    ...
    fn = function(t) {
      ...
      process = function(data) {
        ...
        if (window.$ !== void 0 && self.get_names().length === Object.keys(textures).length) {
...
Shader = (function() {
  ...
  Shader.prototype.setup = function(gl, ind, obj) {
    var attr, buffer, color0, color1, color2, program, r0, r1, resolutionLocation, s0, s1, t, tex0, tex1, texCoordBuffer, texCoordBuffer2, texCoordLocation, x, y;
    program = this.programs[ind];
    gl.useProgram(program);
    resolutionLocation = gl.getUniformLocation(program, "u_resolution");
    gl.uniform2f(resolutionLocation, obj.u_resolution.width, obj.u_resolution.height);
    x = obj.u_resolution.width / 300.0;
    y = obj.u_resolution.height / 300.0;
    texCoordLocation = gl.getAttribLocation(program, "a_texCoord0");
    texCoordBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, texCoordBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([0.0, 0.0, x, 0.0, 0.0, y, 0.0, y, x, 0.0, x, y]), gl.STATIC_DRAW);
